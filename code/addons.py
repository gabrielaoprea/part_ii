...
def mom_occ_(mf, occorb, setocc):
    ...
    HF/KS.'''
    from pyscf.scf import uhf, rohf, hf
    if isinstance(mf, uhf.UHF):
    ...
    def get_occ(mo_energy=None, mo_coeff=None):
        ...
        if isinstance(mf,hf.RHF):
            mo_occ = numpy.zeros_like(setocc)
            nocc = int(numpy.sum(setocc))
            #print('mo_coeff')
            #print(mo_coeff)
            #print('cof_occ')
            #print(coef_occ)
            s = reduce(numpy.dot, (coef_occ.T, mf.get_ovlp(), mo_coeff))
            idx = numpy.argsort(numpy.einsum('ij,ij->j', s, s))[::-1]
            mo_occ[idx[:int(nocc/2)]] = 2.


            log.debug(' New occ pattern: %s', mo_occ)
            if isinstance(mf.mo_energy, numpy.ndarray) and mf.mo_energy.ndim == 1:
                log.debug1(' Current mo_energy(sorted) = %s', mo_energy)
            if (int(numpy.sum(mo_occ)) != nocc):
                log.error('mom electron occupation numbers do not match: %d, %d',
